...
def alib_squeezeSeg_config():
    """Specify the parameters to tune below."""
    mc                    = base_model_config('KITTI')

    #mc.CLASSES            = ['unknown', 'car', 'pedestrian', 'cyclist']
    mc.CLASSES            = ['Dontcare', 'cyclist', 'tricycle', 'smallMot', 'bigMot', 'pedestrian', 'crowds', 'unkonwn']
    mc.NUM_CLASS          = len(mc.CLASSES)
    mc.CLS_2_ID           = dict(zip(mc.CLASSES, range(len(mc.CLASSES))))
    mc.CLS_LOSS_WEIGHT    = np.array([1/15.0, 1.0,  10.0, 10.0, 1/15.0, 1.0,  10.0, 10.0])
    ...
    mc.LCN_WIDTH          = 5
    mc.RCRF_ITER          = 3
    mc.BILATERAL_THETA_A  = np.array([.9, .9, .6, .6, .9, .9, .6, .6])
    mc.BILATERAL_THETA_R  = np.array([.015, .015, .01, .01, .015, .015, .01, .01])
    mc.BI_FILTER_COEF     = 0.1
    mc.ANG_THETA_A        = np.array([.9, .9, .6, .6, .9, .9, .6, .6])
